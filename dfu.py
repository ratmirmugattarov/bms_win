...
def check_status(stage, expected):
    ...
    if status != expected:
        raise SystemExit(
            "DFU: %s failed (%s)"
            % (stage, __DFU_STATUS_STR.get(status, status))
        )
...
def mass_erase():
    ...
    __dev.ctrl_transfer(
        0x21, __DFU_DNLOAD, 0, __DFU_INTERFACE, "\x41", __TIMEOUT
    )
...
def write_memory(addr, buf, progress=None, progress_addr=0, progress_size=0):
    ...
    while xfer_bytes < xfer_total:
        ...
        if progress and xfer_count % 2 == 0:
            progress(
                progress_addr,
                xfer_base + xfer_bytes - progress_addr,
                progress_size,
            )
...
def get_dfu_devices(*args, **kwargs):
    ...
    return list(
        usb.core.find(*args, find_all=True, custom_match=FilterDFU(), **kwargs)
    )
            ...
            if not mass_erase_used:
                for segment in mem_layout:
                    if addr >= segment["addr"] and addr <= segment["last_addr"]:
                        ...
                        break
            write_memory(
                addr, data[:write_size], progress, elem_addr, elem_size
            )
...
def main():
    ...
    parser.add_argument(
        "--vid", help="USB Vendor ID", type=lambda x: int(x, 0), default=None
    )
    parser.add_argument(
        "--pid", help="USB Product ID", type=lambda x: int(x, 0), default=None
    )
    parser.add_argument(
        "-m",
        "--mass-erase",
